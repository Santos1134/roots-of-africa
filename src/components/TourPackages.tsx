...
const regularTours = tours.filter(t => t.id !== 'custom');
const customTour = tours.find(t => t.id === 'custom');
    ...
    <section id="stories" className="py-12 md:py-20 bg-warm-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {[...regularTours].sort((a, b) => a.price - b.price).map((tour, index) => (
            <motion.div
              key={tour.id}
          ...
          {customTour && (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-50px' }}
              transition={{ duration: 0.5, delay: regularTours.length * 0.1 }}
              className="group bg-white rounded-sm overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300"
            >
              <Link to="/book?tour=custom" className="block">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img
                    src={customTour.image}
                    alt={customTour.name}
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                  />
                  <div className="absolute top-3 right-3 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-sm shadow-md">
                    <span className="font-serif text-sm text-deep-brown font-medium">
                      Price on request
                    </span>
                  </div>
                </div>
